...
private final class MockBuzzBuddyAPI: BuzzBuddyAPIProtocol {
    private let contact = DDContactOut(
        id: "contact-1",
        userId: "user-1",
        name: "DD",
    ...
    var createUserResult: Result<UserOut, Error> = .success(
        UserOut(
            id: "user-1",
            name: "Test",
            ddContacts: [
...
private final class InMemoryOwnerCredentialStore: OwnerCredentialStoring {
    private var tokens: [String: String] = ["user-1": "owner-token"]
...
private extension AppState {
    convenience init(
        api: BuzzBuddyAPIProtocol,
        persistence: PersistenceStore,
        locationProvider: LocationProviding
    ) {
        let defaults = UserDefaults(suiteName: UUID().uuidString)!
        let contact = DDContactOut(
            id: "contact-1",
            userId: "user-1",
            name: "DD",
            phoneNumber: "+14155550100",
        ...
        defaults.set(try! JSONEncoder().encode([contact]), forKey: "buzzbuddy.ownerContacts")
        defaults.set(contact.id, forKey: "buzzbuddy.selectedContactId")
        self.init(
            api: api,
            persistence: persistence,
            locationProvider: locationProvider,
            ownerCredentials: InMemoryOwnerCredentialStore(),
            defaults: defaults
        )
    }
}
